...
library(tidyverse)
#library(vsgseqtools) I haved added all the functions from this package into the RMD
library(ggpubr)
library(pheatmap)
library(ggsci)
library("gridExtra")
...
vsg_expand <- function(df, col1, col2, merge = TRUE) {
  ...
  clusters <- df %>%
    vsg_distinct_vectorize(df[col2])
...
```{r}
# These clusters which all have a top hit of Tb427 VSG-3303 do not hit an actual VSG. They map to internal chromosome sections and ribosomal subunits which i determined by blasting the assembled ORF. These types of sequences are often a contaminant that looks like a VSG. For further analysis, I will filter these VSG assembly errors out. In Monica's paper and our current VGS-Seq pipeline these are often filtered out but for some reason this specific sequence got through the filters that are in the VSG-Seq pipeline. 
...
mouse1_assem_vsgs_histo_dat <- results %>%
  filter(sort == "tdTom") %>%
  filter(str_detect(cellid_2, "Lung_R1") != T,
         str_detect(cellid_2, "Lung_Antat_R1") != T) %>%
  filter(mouse == "mouse1") 

write_csv(mouse1_assem_vsgs_histo_dat, "../source_data/ext_dat_5a-1.csv")

mouse1_assem_vsgs_histo <- mouse1_assem_vsgs_histo_dat %>%
...
ggsave("figures/extended_data_figure_5a-1.pdf", mouse1_assem_vsgs_histo,
       device = "pdf", width = 8, height = 6, units = "in")

mouse2_assem_vsgs_histo_dat <- results %>%
  filter(sort == "tdTom") %>%
  filter(str_detect(cellid_2, "Lung_R1") != T,
         str_detect(cellid_2, "Lung_Antat_R1") != T) %>%
  filter(mouse == "mouse2") 

write_csv(mouse2_assem_vsgs_histo_dat, "../source_data/ext_dat_5a-2.csv")

mouse2_assem_vsgs_histo <- mouse2_assem_vsgs_histo_dat %>%
  ggplot(aes(x =  number_of_vsgs_in_sample)) +
  geom_histogram(binwidth = 1) +
  facet_wrap("source", scales = "free_y") +
  theme_pubr(base_size = 20)+
...
ggsave("figures/extended_data_figure_5a-2.pdf", mouse2_assem_vsgs_histo,
       device = "pdf", width = 8, height = 6, units = "in")
```
...
write_csv(eatro_histo_dat, "../source_data/ext_dat_5b.csv")
...
ggsave("figures/extended_data_fig_5b-1.pdf", mouse1_eatro_histo,
       device = "pdf", width = 8, height = 6, units = "in")
...
ggsave("figures/extended_data_fig_5b-1.pdf", mouse2_eatro_histo,
       device = "pdf", width = 8, height = 6, units = "in")
```
...
write_csv(eatro_violin_dat, "../source_data/ext_dat_5c.csv")
...
ggsave("figures/extended_data_fig_5c.pdf", eatro_violin_plot,
       device = "pdf", width = 8, height = 6, units = "in")
```
...
eatro_and_assembled_counts_bar_dat <- eatro_and_assembled_counts %>%
   filter(sort == "tdTom",
        mouse != "invitro") %>%
  select(-main_vsg) %>%
  distinct() %>%
...
write_csv(eatro_and_assembled_counts_bar_dat, "../source_data/ext_dat_5e.csv")

eatro_and_assembled_counts_bar <- eatro_and_assembled_counts_bar_dat %>%
  ggplot(aes(x = source, fill = cell_description)) + 
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  facet_wrap("mouse", scales = "free_x") +
...
ggsave("figures/extended_data_5e.pdf", eatro_and_assembled_counts_bar,
       device = "pdf", width = 12, height = 6, units = "in")
...
supplementary_table_1 <- eatro_and_assembled_counts %>%
  ...
  mutate(assembled_vsgs = case_when(is.na(assembled_vsgs) == T ~ 0,
                                    T ~ assembled_vsgs)) %>%
  dplyr::rename(number_of_VSGs_detected_by_de_novo_assembly = assembled_vsgs,
...
supplementary_table_3 <- dominant_vsg_results %>%
  ...
  dplyr::rename(dominant_vsg = main_vsg, 
         cell = cellid_2,
...
write_csv(cluster59.track.df, "../source_data/ext_dat_5d-1.csv")
...
write_csv(vsg60.track.df, "../source_data/ext_dat_5d-2.csv")
...
write_csv(vsg72.track.df, "../source_data/ext_dat_5d-3.csv")
...
ggsave("figures/vsg72.coverage.pdf", vsg72.coverage,
       device = "pdf", width = 8, height = 4, units = "in")
```
Thats the end of all the analyses! 

# Tidying

```{r}
# calculating the total cells and creating a list of all the sequencing files used ( this is to be used for annotating the SRA upload with all our filanames and metadata)
SS3x_007_EATRO_all_cell_names <- read_delim("VSG_expression_matrices/SS3x_007_EATRO_VSG_relative_expression_table_raw.tsv", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  dplyr::rename(gene_id = ...1)  %>%
  pivot_longer(cols = A1_Lung_R1:last_col(), names_to = "cell" ,values_to = "Percent") %>% 
  select(cell) %>%
  distinct(cell) %>%
...
SS3x_008_EATRO_all_cell_names <- read_delim("VSG_expression_matrices/SS3x_008_EATRO_VSG_relative_expression_table_raw.tsv", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  dplyr::rename(gene_id = ...1) 
  pivot_longer(cols = A1_mouse1_blood:last_col(), names_to = "cell" ,values_to = "Percent") %>%
  select(cell) %>%
  distinct() %>%
  separate(cell, c("well", "mouse","source"), sep = "_", remove = F) %>%
  mutate(experiment = "exp2",
...
all_cells <- rbind(SS3x_007_EATRO_all_cell_names, SS3x_008_EATRO_all_cell_names)

water_wells <- c("C3", "G2", "L1", "P1","N4","M8", "H10","E11","O12","J14","K15","O19", "F22","D24")

all_analyzed_cells <- all_cells %>%
  filter(sort == "tdTom",
         mouse != "invitro") %>%
   filter(str_detect(cell, "Lung_R1") != T,
         str_detect(cell, "Lung_Antat_R1") != T) %>% 
  filter(!well %in% water_wells )

all_sequenced_cell_filenames <- read_csv("VSG_expression_matrices/all_sequenced_cell_filenames.txt", 
    col_names = FALSE) %>%
  dplyr::rename(cell = X1) %>%
  filter(cell != "unassigned.fq.gz")

ss307_filenames <- read_csv("ss307_filenames.txt", 
    col_names = FALSE) %>%
  dplyr::rename(filename = X1) %>%
   separate(filename, "name", sep = "_R", remove = F) %>%
  separate(name, c("well", "source"), sep = "_") %>%
  mutate(sort = case_when(source == "AnTat" ~ "Antat",
                          TRUE ~ "tdTom")) %>%
  mutate(sort = case_when(str_detect(filename, "Lung_Antat") == TRUE & str_detect(well, "A|B|C|D") == TRUE ~ "Antat",  TRUE ~ sort)) %>%
  mutate(source = case_when(source == "AnTat" ~ "Mix",
                            TRUE ~ source)) %>%
  mutate(experiment = "exp1") %>%
  mutate(mouse = case_when(source == "Mix" ~ "invitro", 
...
ss308_filenames <- read_csv("ss308_filenames.txt", 
    col_names = FALSE) %>%
  dplyr::rename(filename = X1) 
  filter(str_detect(filename, "Water") != T) %>%
  filter(filename != "unassigned.fq.gz") %>%
  separate(filename, c("mouse", "source","cell_1","cell_2", "well", "plate", "experiment"), sep = "_", remove = F) %>%
  mutate(experiment = "exp2",
         sort = "tdTom")  %>%
  select(-cell_1, -cell_2, -plate ) %>%
  distinct()

all_filenames <- rbind(ss307_filenames,ss308_filenames) %>%
  distinct() %>%
  filter(str_detect(filename, "Lung_R1") != T) %>%
  filter(str_detect(filename, "Lung_Antat_R1") != T)

all_analyzed_cell_filenames <- right_join(all_filenames, all_analyzed_cells)
 
#write_tsv(all_analyzed_cell_filenames, "all_analyzed_cell_filenames.txt")
