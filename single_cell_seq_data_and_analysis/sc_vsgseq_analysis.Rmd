...
ss3x07_sc_results <- vsg_read("ss3x_07_results/results_files/all_results.txt") %>%
  select(-well, -R) %>%
  filter(type != "Water") %>%
  rename(cell = type) %>%
...
ss3x07_sc_results_renamed <- vsg_rename(
  ss3x07_sc_results, 
  "all_vsg_orfs/cluster_reference_table.txt") 
...
read_counts <- read_table("read_counts.txt", 
    col_names = FALSE) %>%
  rename(lines = X1,
         file = X2) %>%
...
in_vitro <- results %>% 
  filter(source == "mix") %>% 
  mutate(vsg_of_interest = case_when(cluster == "Cluster 9" ~ "AnTat",
                                     cluster == "Cluster 25" ~ "VSG-60",
                                     cluster == "Cluster 21" ~ "AnTat variant",
                                     TRUE ~ "other VSG")) %>%
  mutate(vsg_of_interest = factor(vsg_of_interest, levels = c("AnTat", "VSG-60", "AnTat variant","other VSG"))) %>%
  ggplot(aes(x = vsg_of_interest, y = Percent)) +
  geom_point(position = position_jitter(width = 0.4, height = 0.6)) +
  theme_pubr() 
