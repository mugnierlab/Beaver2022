...
mouse_names <- tibble(mouse = list("M12","M27", "M32", "M33", "M30", "M36", "M4", "M6", "M2", "M34","M35", "M3"),
                      mouse_final = 1:12) #this is the final names for mice presented in the paper
...
ggsave("extended_data_fig_3b.pdf",Supplementary_figure_1b,
       device = "pdf", width = 6, height = 4, units = "in")

write.csv(all_predictions, "parasitemia_predictions.csv")
```
# Extended data fig 3c
## Total parasite load per tissue
```{r}
...
ggsave("figures/extended_data_fig_3c.pdf",total_parasite_load_graph,
       device = "pdf", width = 6, height = 4, units = "in")
```


# Extended data fig 3d
## Plate standards correlation
```{r}
...
ggsave("figures/extended_data_fig_3d.pdf", all_standards_graph,
...
```{r}
# Starting to analyze the VSGSeq data
...
ggsave("figures/extended_data_fig_4b.pdf",vsgs_vs_parasites_plot,
       device = "pdf", width = 8, height = 6, units = "in")
```
...
# Figure 3b and extended data table 1
...
tab_df(supplement_table_dat, alternate.rows = T, # this colors the rows 
       ...
       sort.column = 2,
       file = "figures/extended_data_table_1.doc")
...
ggsave("figures/figure_3b.pdf", quant_plot,
       device = "pdf", width = 8, height = 6, units = "in")
...
# Note the mouse names from AID -/- and WT get confusing. But I kept the strain throughout
...
ggsave("figures/figure_5A.pdf", wt_and_aid_starting_vsg_espression_plot,
       device = "pdf", width = 8, height = 6, units = "in")
```
...
write_csv(wt_and_aid_vsg_counts, "source_data/5b_source.csv")
...
# I had to add the p values manually becuase the facet wrap messes up how R plots things. The P-values can be found in the data frame above

ggsave("figures/figure_5B.pdf", wt_and_aid_vsg_count_plot,
       device = "pdf", width = 8, height = 6, units = "in")
```
...
elisa_plot <- elisa_dat %>%
  ...
  xlab("Time post infection (days)")
elisa_plot
ggsave("figures/extended_data_fig_6a.pdf", elisa_plot,
       device = "pdf", width = 8, height = 6, units = "in")
```
...
ggsave("figures/extended_data_fig_4c.pdf",vsgs_vs_pad1,
       device = "pdf", width = 8, height = 6, units = "in")
```
...
ggsave("figures/figure_4a.pdf", tsetse_VSGs_count_plot,
       device = "pdf", width = 8, height = 6, units = "in")
```
...
ggsave("figures/figure_4b.pdf", tsetse_d14_counts_by_space_plot,
       device = "pdf", width = 8, height = 6, units = "in")
```
...
ggsave("figures/figure_4c.pdf", tsetse_starting_vsg_espression_plot,
       device = "pdf", width = 8, height = 6, units = "in")
```

# Figure 4d
```{r}
# combining with salivary gland sequencing results and use updated cd-hit clustering to include these mVSGs 
tsetse_sg_raw_results <- rbind(tsetse_raw_results, sg_raw_results)
...
ggsave("figures/figure_4d.pdf", percent_sg_vsgs,
       device = "pdf", width = 8, height = 6, units = "in")
```
